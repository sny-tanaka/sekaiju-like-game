...
## 18. スキルLv反映バグ修正（エンジン修正・バランス再調整）

### 18.1 バグの内容

`src/domain/battle.ts` の味方スキル解決・ユニオンスキル解決において、スキルレベルが常に **Lv=1** として処理されていた。スキル効果は `power:(lv)=>...` のようにLv連動で定義されているにも関わらず、`learnedSkills` に記録されている学習Lvが戦闘に反映されていなかった。
...
#### (a) `src/domain/types.ts` `Combatant` に追加
```ts
/** 味方の学習スキルLv（戦闘でスキル威力/消費に反映。敵は未使用）。戦闘中のみ。 */
skillLevels?: Record<SkillId, number>;
```

#### (b) `src/domain/battle.ts` `buildAlly` に追加
```ts
skillLevels: char.learnedSkills,
```

#### (c) 味方スキル解決（`cmd.kind==='skill'` 内）を修正
```ts
const level = actor.skillLevels?.[cmd.skillId] ?? 1;
```

#### (d) ユニオン解決 `resolveUnion` を修正
```ts
const level = activator.skillLevels?.[cmd.unionSkillId] ?? 1;
```

**注意事項:**
- 敵スキルの `applySkillEffect(..., 1, ...)` は変更しない（敵はLv概念なし）
- アイテムの `amount(1)` も変更しない
- パッシブ計算は既に学習Lv連動なので変更不要

### 18.3 シミュのスキルLvモデル

忠実シミュ（`scripts/balanceSimFaithful.ts`・`src/domain/balanceSim.test.ts`）の標準パーティのスキル学習Lvを、適正キャラLvに応じて設定:

```
skillLv = clamp(1 + floor((charLv - 1) / 8), 1, skill.maxLevel)
```

| キャラLv | スキルLv例（maxLevel=5） | 適用ボス階 |
| --- | --- | --- |
| 12 | 2 | F10 |
| 23 | 3 | F20 |
| 35 | 5 | F30 |
...
変更なし。AC5（残りグラインド3〜5Lv）は変更前後で同一（tier1雑魚exp=100, 9階×4エンカ×2体=7200exp, Lv12→19, 残り4Lv）。
