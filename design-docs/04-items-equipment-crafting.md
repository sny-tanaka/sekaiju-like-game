...
> - **採集スキルは「職業スキル」として提供**（採掘=戦士、伐採=守護兵、収穫=魔導士、採取/釣り/狩猟=狩人）。設計上は「種族スキル」だが、現データモデルは種族が `unionSkillTree`（ユニオン専用）のみを持ち汎用の種族スキルツリーが無いため、職業ツリーに配置した。採集適性の種族分けは Phase 6 で要再検討（種族汎用ツリー導入 or この方針を正式採用）。
