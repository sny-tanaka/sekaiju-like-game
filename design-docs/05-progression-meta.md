...
> **オートセーブ契機（正準・[06 §6](./06-tower-progression.md) と統一）**: ①拠点帰還時 ②階層移動（階段/ワープ）時 ③階層ボス撃破時 ④全滅処理後（`diveState=null` 化）。戦闘中の毎ターン保存はしない（戦闘は実行時状態 `BattleState`。中断したい場合のみ、戦闘開始直前の `diveState` から再戦する設計でよい）。
