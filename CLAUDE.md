...
- **新しく実装・修正したロジックの正しさは、原則として単体テスト (vitest) で検証する**。
  「既存の N 個のテストが緑」だけでは、その PR で**追加・変更したロジックを検証したことにならない**。
  PR ごとに、変更点に対応する **新規 / 更新テスト** が `git diff` に含まれているのが基本状態。
- 単体テスト化が必須のもの:
  - 純粋関数（domain/, helper、計算ロジック、リデューサ）
  - state 遷移（モーダル開閉、フラグ ON/OFF、永続化の load/save）
  - 条件分岐（特定条件で表示/非表示、特定値で disable）
  - バグ修正（修正前に **fail する再現テスト** を書いてから直す）
- 単体テスト化を省略してよいもの:
  - SCSS / レイアウトのみの調整（display, padding, color など）
  - アニメーション・配置の見た目調整（モチベが「Storybook で見て直す」もの）
  - ビルド設定・依存追加など、ロジックが介在しない変更
- 上記の境界が曖昧な場合は **テストを書く側に倒す**。書いてから「テスト不能だった」と判明することはあるが、
  書かずに「テストできたのに省いた」になることが多い。
- ディレクターがサブエージェントに指示を出すときは、「実装と同じコミットで対応する単体テストを追加すること」を
  **指示書に明記**する。検証ゲートで「既存テスト pass」を確認しても、新規テストの欠落は気づけない。
