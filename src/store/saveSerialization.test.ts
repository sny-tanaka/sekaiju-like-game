...
import { deriveSlotMeta, deserializeSave, serializeSave } from '@/store/saveSerialization';

function makeSave(name = 'ギルド') {
  const save = createInitialSaveData(name, createStarterParty(createRng(1)));
  save.savedAt = 1234;
...
describe('saveSerialization', () => {
  ...
  test('serializeSave は元データを共有しない（ディープコピー）', () => {
    ...
    copy.guild.members[0].name = '変更後';
    expect(save.guild.members[0].name).not.toBe('変更後');
  });
  ...
  test('未知の未来バージョンは破損扱い（上書き防止）', () => {
    ...
    const result = deserializeSave(future);
    expect(result.ok).toBe(false);
  });

  test('deriveSlotMeta はギルド名・到達階・セーブ時刻を抽出する', () => {
    const save = makeSave('わがギルド');
    const meta = deriveSlotMeta(2, save);
