...
describe('装備の周回グレード倍率（+5 ティア相当）', () => {
  test('gradeMult は指数式: grade=1→1.0、grade=2→1.62^5≈10.66', () => {
    ...
    // equip_short_sword は atk8 → grade2 で round(8 * 1.62^5) ≈ 85
    expect(gradedBaseBonuses('equip_short_sword', 2).atk).toBeGreaterThanOrEqual(80);
    expect(gradedBaseBonuses('equip_short_sword', 2).atk).toBeLessThanOrEqual(90);
  });
});
...
describe('素材売却での装備グレード解放', () => {
  test('grade2 素材を売ると装備が Lv2 で並び、その個体が作れる', () => {
    let s = gw();
    s = { ...s, guild: { ...s.guild, gold: 10000 } }; // Lv2 装備価格が gradeMult(2)≈10.66倍になるため増額
