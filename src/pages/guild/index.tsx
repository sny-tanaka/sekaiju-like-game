...
import type { RaceId } from '@/domain/types';
...
export const Page = () => {
  ...
  const handleCreate = useCallback(async () => {
    ...
    // 開始時は種族の既定職業（転職は到達階で解放）。
    const classId = RACES[raceId].defaultClassId;
          ...
          >
            {raceIds.map((id) => (
              <option
                ...
                value={id}
              >
                {RACES[id].name}（{CLASSES[RACES[id].defaultClassId].name}）
              </option>
            ))}
          </select>
        </label>
